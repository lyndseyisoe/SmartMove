...
export default function Movers() {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 fade-up">
      ...
      <div className="flex max-w-xl items-center gap-3 rounded-2xl border border-[var(--color-border)] bg-white px-4 py-3 shadow-[var(--shadow-card)] focus-within:border-[var(--color-teal)] focus-within:ring-2 focus-within:ring-[var(--color-teal-light)]"><Search className="size-5 text-[var(--color-slate)]" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by mover name" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-[var(--color-slate)]" /></div>

...
import { ArrowLeft, CheckCheck, Image, MapPin, MessageCircle, MoreHorizontal, Phone, Plus, Send, Smile, Truck } from 'lucide-react';
...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    messagesApi.list(selected.bookingId).then((items) => setMessages(items.map((item) => ({ id: item.id, from: item.sender_id === user?.id ? 'me' : 'them', text: item.body, time: new Date(item.created_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }) }))));
  }, [selected, user?.id]);
  ...
  const sendMessage = (event) => {
    ...
    messagesApi.send(selected.bookingId, text).then((message) => {
      setMessages((current) => [...current, { id: message.id, from: 'me', text: message.body, time: 'Now' }]);
        ...
        <aside className={`${selected ? 'hidden md:flex' : 'flex'} w-full shrink-0 flex-col border-r border-[var(--color-border)] md:w-[310px]`}>
          ...
          <div className="flex-1 overflow-y-auto p-2">
            ...
            {chats.map((chat) => <button key={chat.id} onClick={() => setSelected(chat)} className={`flex w-full gap-3 rounded-xl p-3 text-left transition ${selected?.id === chat.id ? 'bg-[var(--color-teal-light)]' : 'hover:bg-[var(--color-bg)]'}`}>
              <div className={`relative flex size-11 shrink-0 items-center justify-center rounded-full text-xs font-bold ${chat.color}`}>{chat.initials}{chat.online && <span className="absolute bottom-0 right-0 size-3 rounded-full border-2 border-white bg-[#32bd7a]" />}</div>

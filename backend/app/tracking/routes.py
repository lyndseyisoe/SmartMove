...
def accessible_booking(booking_id, user_id):
    """A tracking item's booking is only visible to its client or its
    assigned mover - same rule messages/routes.py uses for conversations."""
    return Booking.query.filter(
        Booking.id == booking_id,
        db.or_(Booking.client_id == user_id, Booking.mover_id == user_id),
    ).first()
...
def get_tracking_items(booking_id):
    ...
    if accessible_booking(booking_id, user_id) is None:
        return jsonify({"error": "Booking not found"}), 404

    items = (
        TrackingItem.query
        .filter_by(booking_id=booking_id)
        .order_by(TrackingItem.created_at.asc())
        .all()
    )
...
def update_tracking_item(item_id):
    ...
    if item is None or accessible_booking(item.booking_id, user_id) is None:
